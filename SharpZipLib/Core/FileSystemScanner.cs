...
namespace ICSharpCode.SharpZipLib.Core
{
	...
	public class FileSystemScanner
	{
        ...
        void ScanDir(string directory, bool recurse)
        {
            ...
            try
            {
                List<string> names = new List<string>(System.IO.Directory.EnumerateFiles(directory));
                    //GetFiles(directory);
                bool hasMatch = false;
                for (int fileIndex = 0; fileIndex < names.Count; ++fileIndex)
                {
            ...
            if (alive_ && recurse)
            {
                try
                {
                    List<string> names = new List<string>(System.IO.Directory.EnumerateDirectories(directory));
